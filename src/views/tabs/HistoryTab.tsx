...
export default function HistoryTab() {
  ...
  // Build unified history from ALL data arrays (not just timeline)
  const allEntries = useMemo(() => {
    const items: { id: string; action: string; details: string; category: string; createdAt: string }[] = [];

    state.dailyLogs.forEach(l => items.push({
      id: l.id, action: l.text, details: `Tagged: ${l.tag}`, category: l.tag, createdAt: l.createdAt,
    }));
    state.expenses.forEach(e => items.push({
      id: e.id, action: `${formatINR(e.amount)} - ${e.category}`, details: e.note || '', category: 'expense', createdAt: e.createdAt,
    }));
    state.foodEntries.forEach(f => items.push({
      id: f.id, action: `${f.name} (${f.portionSize})`, details: `${f.mealType}${f.calories ? ' · ' + f.calories + ' cal' : ''}`, category: 'food', createdAt: f.createdAt,
    }));
    state.sleepEntries.forEach(s => items.push({
      id: s.id, action: `${s.hours}h sleep - ${s.quality}`, details: s.bedTime && s.wakeTime ? `${s.bedTime} → ${s.wakeTime}` : '', category: 'sleep', createdAt: s.createdAt,
    }));
    state.activities.forEach(a => items.push({
      id: a.id, action: `${a.steps.toLocaleString()} steps`, details: a.distanceKm ? `${a.distanceKm} km` : '', category: 'exercise', createdAt: a.createdAt,
    }));
    state.reminders.forEach(r => items.push({
      id: r.id, action: r.title, details: r.isCompleted ? 'Completed' : `Due: ${formatDate(r.dateTime)}`, category: 'reminder', createdAt: r.createdAt,
    }));
    state.listItems.forEach(l => items.push({
      id: l.id, action: l.title, details: `${l.listType}${l.rating ? ' · ★' + l.rating : ''}`, category: 'list', createdAt: l.createdAt,
    }));

    return items.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [state.dailyLogs, state.expenses, state.foodEntries, state.sleepEntries, state.activities, state.reminders, state.listItems]);
  ...
  const dayActivities = useMemo(() => {
    ...
    return allEntries.filter(item => {
      ...
      const withinDate = isWithinInterval(d, { start, end });
      const matchesCat = filterCategory === 'all' || item.category === filterCategory;
      const matchesSearch = !searchQuery || item.action.toLowerCase().includes(searchQuery.toLowerCase()) || item.details.toLowerCase().includes(searchQuery.toLowerCase());
      return withinDate && matchesCat && matchesSearch;
    });
  }, [allEntries, selectedDate, filterCategory, searchQuery]);
