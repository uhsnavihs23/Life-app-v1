...
export default function TodayTab() {
  const { state, addLog, addExpense, addFood, addSleep, addActivity, addHealthMetrics, addListItem, addFile, dispatch } = useApp();
    ...
    <div className="pb-4 fade-in safe-area-top">
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        ...
        <div className="card p-3 text-center">
          ...
          <p className="text-lg font-bold text-emerald-500">{todayActivity.reduce((s, a) => s + a.steps, 0).toLocaleString()}</p>
        </div>
      </div>
        ...
        <div className="card p-4 mb-4 slide-up">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold" style={{ color: 'var(--color-text)' }}>
              {quickAdd === 'expense' && '💰 Add Expense'}{quickAdd === 'food' && '🍽️ Add Food'}
              {quickAdd === 'sleep' && '😴 Add Sleep'}{quickAdd === 'activity' && '🏃 Add Activity'}
              {quickAdd === 'health' && '❤️ Health Check'}{quickAdd === 'list' && '📝 Add to List'}
            </h3>
          ...
          {quickAdd === 'expense' && (
            <div className="space-y-3">
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-semibold" style={{ color: 'var(--color-text-secondary)' }}>₹</span>
          ...
          {quickAdd === 'food' && (
            <div className="space-y-3">
              ...
              <input className="ios-input" placeholder="Portion (e.g., 1 plate, 200ml)" value={foodPortion} onChange={e => setFoodPortion(e.target.value)} />
              <input className="ios-input" type="number" inputMode="numeric" placeholder="Calories (optional)" value={foodCalories} onChange={e => setFoodCalories(e.target.value)} />
              <div className="flex gap-2 flex-wrap">
          ...
          {quickAdd === 'sleep' && (
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: 'var(--color-text-secondary)' }}>Quality</label>
                <div className="flex gap-2 flex-wrap">
          ...
          {quickAdd === 'activity' && (
            <div className="space-y-3">
              ...
              <input className="ios-input" type="number" inputMode="decimal" placeholder="Distance in km (optional)" value={actDistance} onChange={e => setActDistance(e.target.value)} step="0.1" />
          ...
          {quickAdd === 'health' && (
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text-secondary)' }}>Energy: <strong>{healthEnergy}/10</strong></label>
                <input type="range" min="1" max="10" value={healthEnergy} onChange={e => setHealthEnergy(e.target.value)} className="w-full accent-rose-500" />
              </div>
              <div className="flex items-center gap-2">
                <Droplets className="w-5 h-5 text-blue-500 flex-shrink-0" />
                <input className="ios-input flex-1" type="number" inputMode="numeric" placeholder="Glasses of water" value={healthWater} onChange={e => setHealthWater(e.target.value)} />
              </div>
              ...
              <div className="flex gap-2">
                {(['movie', 'music', 'book'] as const).map(t => (
                  <button key={t} className={`flex-1 py-2 rounded-xl text-sm font-medium ${listType === t ? 'bg-indigo-500 text-white' : ''}`}
                    style={listType !== t ? { background: 'var(--color-surface-alt)', color: 'var(--color-text-secondary)' } : {}} onClick={() => setListType(t)}>
                    {t === 'movie' ? '🎬' : t === 'music' ? '🎵' : '📚'} {t.charAt(0).toUpperCase() + t.slice(1)}
                  </button>
                ))}
              </div>
              <input className="ios-input" placeholder={`${listType.charAt(0).toUpperCase() + listType.slice(1)} title...`} value={listTitle} onChange={e => setListTitle(e.target.value)} autoFocus />
              <button className="ios-btn ios-btn-primary w-full" onClick={handleSubmitList} disabled={!listTitle.trim()}><Plus className="w-4 h-4" /> Add</button>
            </div>
          )}
        </div>
      )}
      ...
      {/* Today's Feed */}
      <div className="mb-2">
        <h2 className="text-lg font-semibold mb-3" style={{ color: 'var(--color-text)' }}>Today's Feed ({allTodayItems.length})</h2>
        {allTodayItems.length === 0 ? (
          ...
          <div className="space-y-2">
            {displayItems.map((item) => {
              ...
              return (
                <SwipeableCard key={item.id} onDelete={() => handleDeleteItem(item)}>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: config.bg }}>
                      <span className="text-sm">{config.emoji}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm" style={{ color: 'var(--color-text)' }}>{'text' in item ? item.text : ''}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: config.bg, color: config.color }}>{config.label}</span>
                        <span className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>{formatTime(item.createdAt)}</span>
                      </div>
                    </div>
                  </div>
                </SwipeableCard>
            ...
            {allTodayItems.length > 8 && (
              <button className="w-full py-2 text-sm font-medium flex items-center justify-center gap-1"
                style={{ color: 'var(--color-primary)' }} onClick={() => setShowAllEntries(!showAllEntries)}>
                {showAllEntries ? <><ChevronUp className="w-4 h-4" /> Show Less</> : <><ChevronDown className="w-4 h-4" /> Show All ({allTodayItems.length})</>}
              </button>
            )}
          </div>
        )}
